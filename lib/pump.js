...
export async function pumpCheck (connection, signature) {
  const tx = await connection.getTransaction(signature)
  const balanceChange = (tx.meta.postBalances[0] - tx.meta.preBalances[0]) / LAMPORTS_PER_SOL
  const outBalance = tx.meta.postBalances[0] / LAMPORTS_PER_SOL
  console.log(`${balanceChange < 0 ? '\x1b[31mBUY' : '\x1b[32mSELL'} ${balanceChange.toFixed(9)} SOL\x1b[0m ${outBalance.toFixed(9)} SOL https://solscan.io/tx/${signature} \x1b[0m`)
